...
public class TxnController {
	...
	public ResponseEntity<List<Txn>> getTxns(@PathVariable("ahId") Long ahId,
		...
		return ResponseEntity.ok(txnService.getPeriodicTxnByAhId(ahId, start, end));
	}
	...
	private ResponseEntity<Txn> saveTxn(Txn txn, BindingResult results,String operation) throws TxnException, AccountHolderException{
		...
		if(operation.equals("ADD"))
			...
			re = new ResponseEntity<>(txnService.update(txn),HttpStatus.ACCEPTED);
		return re;
